...
class lightningModule(L.LightningModule):
    def __init__(self,lr,batch_size,in_features,layers,p_val,max_epochs,disease,CV,w_decay):
        super().__init__()
        ...
        print(len(layers))
        if len(layers)==2:
            self.encoder = EncoderV1(layers)
            self.decoder = DecoderV1(layers)
        elif len(layers)==3:
            self.encoder = EncoderV2(layers)
            self.decoder = DecoderV2(layers)
        elif len(layers)==4:
            self.encoder = EncoderV3(layers)
            self.decoder = DecoderV3(layers)
        elif len(layers)==5:
            self.encoder = EncoderV4(layers)
            self.decoder = DecoderV4(layers)
